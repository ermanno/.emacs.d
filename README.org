...
#+begin_src bash
{
  ...
  outputs = { self, nixpkgs, home-manager, ... }:
    let
      username = "ermann";
      system = "x86_64-linux";  # Change to "aarch64-darwin" for macOS
    in
    {
...
Install the packages by running from the ~/.config/home-manager directory the following command:

#+begin_src bash
nix run home-manager -- init --switch --flake .
#+end_src
